      ...
      <AnimatedView order={6}>
        <SafeAreaView className="flex-1 justify-items-center mt-10 w-full">
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
              <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={80}>
                <Animated.View entering={FadeIn.duration(1400)} >
                  <BlurView className="px-2 border border-gray-500" intensity={5}>
                    <Animated.View
                ...
                >
                  <TouchableOpacity
                    ...
                    className={`flex-row items-center justify-center bg-amber-200 py-3 rounded-lg ${
                      (loading || email === '' || password === '') && 'bg-slate-400 opacity-60'
                    }`}
                  >
                    {loading ? (
                      <Text className={`font-bold text-center text-lg ${(loading || email === '' || password === '') ? 'text-white' : 'text-black'}`}>
                        <ActivityIndicator />
                      </Text>
                    ) : (
                      <>
                        <Icon
                          ...
                          name="sign-in-alt"
                          type="font-awesome-5"
                          color={(loading || email === '' || password === '') ? 'white' : 'black'}
                          size={20}
                        />
                        <Text className={`font-bold text-center text-lg ${(loading || email === '' || password === '') ? 'text-white' : 'text-black'}`}>
                          Sign In
