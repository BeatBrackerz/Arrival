        ...
        renderItem={({item}) => (
            ...
            >
                <View className={`${!origin && 'opacity-20'}`}>
                    ...
                    />
                    <Text className="mt-2 text-lg font-semibold text-black dark:text-white">{item.title}</Text>
                    <Icon className="p-2 bg-black rounded-full w-10 mt-4 dark:bg-white" type="antdesign" name="arrowright" color={colorScheme === 'light' ? 'white' : 'black'}/>
                </View>
            </TouchableOpacity>
        )}
       />
